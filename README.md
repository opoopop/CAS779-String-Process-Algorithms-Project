...
First 4 dataset have small in order to prove the correctness of the algorithm and it have been proved that the result is correct. So I will only use the last 6 dataset to analysis the speed. Here I use an online jundge system to test the speed because the algorithm only take several seconds so the time of result fluctuate largely if a set a clock on local which would not happen in a online judge system. But I still test each version of algorithm for 10 time and take a average time.  
